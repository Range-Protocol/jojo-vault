///**
// * Submitted for verification at Arbiscan.io on 2023-07-25
// */
//
///*
...
//        require(
//            nonceAfter == nonceBefore + 1, 'SafeERC20: permit did not succeed'
//        );
//    }
//
//    /**
...
//        require(
//            value <= type(uint96).max, "SafeCast: value doesn't fit in 96 bits"
//        );
//        return uint96(value);
//    }
//
...
//        require(
//            value <= type(uint88).max, "SafeCast: value doesn't fit in 88 bits"
//        );
//        return uint88(value);
//    }
//
...
//        require(
//            value <= type(uint80).max, "SafeCast: value doesn't fit in 80 bits"
//        );
//        return uint80(value);
//    }
//
...
//        require(
//            value <= type(uint72).max, "SafeCast: value doesn't fit in 72 bits"
//        );
//        return uint72(value);
//    }
//
...
//        require(
//            value <= type(uint64).max, "SafeCast: value doesn't fit in 64 bits"
//        );
//        return uint64(value);
//    }
//
...
//        require(
//            value <= type(uint56).max, "SafeCast: value doesn't fit in 56 bits"
//        );
//        return uint56(value);
//    }
//
...
//        require(
//            value <= type(uint48).max, "SafeCast: value doesn't fit in 48 bits"
//        );
//        return uint48(value);
//    }
//
...
//        require(
//            value <= type(uint40).max, "SafeCast: value doesn't fit in 40 bits"
//        );
//        return uint40(value);
//    }
//
...
//        require(
//            value <= type(uint32).max, "SafeCast: value doesn't fit in 32 bits"
//        );
//        return uint32(value);
//    }
//
...
//        require(
//            value <= type(uint24).max, "SafeCast: value doesn't fit in 24 bits"
//        );
//        return uint24(value);
//    }
//
...
//        require(
//            value <= type(uint16).max, "SafeCast: value doesn't fit in 16 bits"
//        );
//        return uint16(value);
//    }
//
...
//        require(
//            value <= type(uint8).max, "SafeCast: value doesn't fit in 8 bits"
//        );
//        return uint8(value);
//    }
//
...
//    constructor() Ownable() {
//        domainSeparator =
//            EIP712._buildDomainSeparator('JOJO', '1', address(this));
//    }
//
...
//        address indexed perp, address indexed liquidatedTrader, uint256 fee
//    );
//
//    event HandleBadDebt(
//        address indexed liquidatedTrader,
...
//            (int256 paperAmount, int256 creditAmount) =
//                IPerpetual(state.openPositions[trader][i]).balanceOf(trader);
//            Types.RiskParams storage params =
//                state.perpRiskParams[state.openPositions[trader][i]];
...
//            maintenanceMargin +=
//                (exposureIncrement * params.liquidationThreshold) / Types.ONE;
//
//            unchecked {
//                ++i;
//            }
...
//        returns (bool)
//    {
//        (int256 netPositionValue,, uint256 maintenanceMargin) =
//            getTotalExposure(state, trader);
//
//        // net value >= maintenanceMargin
//        return netPositionValue + state.primaryCredit[trader]
...
//        (int256 netPositionValue,, uint256 maintenanceMargin) =
//            getTotalExposure(state, trader);
//        return netPositionValue + state.primaryCredit[trader] >= 0
//            && netPositionValue + state.primaryCredit[trader]
//                + SafeCast.toInt256(state.secondaryCredit[trader])
//                >= SafeCast.toInt256(maintenanceMargin);
//    }
//
...
//                    }
//                }
//                (int256 paperAmount, int256 credit) =
//                    IPerpetual(perp).balanceOf(trader);
//                maintenanceMargin += (
...
//            if (perp != p) {
//                (int256 paperAmountPrime, int256 creditAmountPrime) =
...
//                );
//                netValuePrime +=
//                    paperAmountPrime.decimalMul(price) + creditAmountPrime;
...
//            );
//        int256 liqPrice = (
//            maintenanceMarginPrime - netValuePrime - creditAmount
...
//        (int256 brokenPaperAmount,) =
//            IPerpetual(perp).balanceOf(liquidatedTrader);
...
//        price =
//            liqtorPaperChange > 0 ? price - priceOffset : price + priceOffset;
//
//        // calculate credit change
//        liqtorCreditChange =
//            -1 * liqtorPaperChange.decimalMul(SafeCast.toInt256(price));
//        insuranceFee =
//            (liqtorCreditChange.abs() * params.insuranceFeeRate) / Types.ONE;
//    }
//
...
//            perp, liquidatedTrader, liqedPaperChange, liqedCreditChange, ltSN
//        );
...
//                int256 creditChange = (paperChange * orderList[i].creditAmount)
...
//                uint256 serialNum =
//                    state.positionSerialNum[orderList[i].signer][msg.sender];
//                emit OrderFilled(
//                    orderHashList[i],
//                    orderList[i].signer,
//                    msg.sender,
...
//            int256 takerFee = SafeCast.toInt256(
//                result.creditChangeList[0].abs()
...
//        int256 temp1 =
//            int256(makerOrder.creditAmount) * int256(takerOrder.paperAmount);
//        int256 temp2 =
//            int256(takerOrder.creditAmount) * int256(makerOrder.paperAmount);
//
...
//        (positionNetValue, exposure, maintenanceMargin) =
//            Liquidation.getTotalExposure(state, trader);
//        netValue = positionNetValue + state.primaryCredit[trader]
//            + SafeCast.toInt256(state.secondaryCredit[trader]);
//    }
//
//    /// @inheritdoc IDealer
...
//            'setOperator(address,bool)', operator, isValid
//        );
//    }
//
...
//    {
//        return abi.encodeWithSignature(
//            'requestWithdraw(uint256,uint256)', primaryAmount, secondaryAmount
//        );
//    }
//
...
//    {
//        return abi.encodeWithSignature(
//            'executeWithdraw(address,bool)', to, isInternal
//        );
//    }
//}
//
...
//
//    event TransferIn(
//        address trader, uint256 primaryAmount, uint256 secondaryAmount
//    );
//
//    event TransferOut(
//        address trader, uint256 primaryAmount, uint256 secondaryAmount
...
//            block.timestamp + state.withdrawTimeLock;
...
//        _withdraw(
//            state, msg.sender, to, primaryAmount, secondaryAmount, isInternal
//        );
//    }
//
//    function _withdraw(
...
//            require(
//                Liquidation._isSolidSafe(state, payer), Errors.ACCOUNT_NOT_SAFE
//            );
//        } else {
...
//        uint256 oldMaxPositionAmount, uint256 newMaxPositionAmount
//    );
//
//    event SetWithdrawTimeLock(
//        uint256 oldWithdrawTimeLock, uint256 newWithdrawTimeLock
//    );
//
//    event SetOrderSender(address orderSender, bool isValid);
//
//    event SetOperator(
//        address indexed client, address indexed operator, bool isValid
//    );
//
...
//        address indexed perp, int256 oldRate, int256 newRate
//    );
//
//    // ========== functions ==========
//
...
//                    state.registeredPerp[i] =
//                        state.registeredPerp[state.registeredPerp.length - 1];
...
//    {
//        require(
//            perpList.length == rateList.length, Errors.ARRAY_LENGTH_NOT_SAME
//        );
...
//    {
//        require(
//            state.validOrderSender[orderSender], Errors.INVALID_ORDER_SENDER
//        );
//
//        /*
//            parse tradeData
...
//                domainSeparator, Trading._structHash(order)
//            );
...
//            state, orderHashList, orderList, matchPaperAmount
//        );
//
//        // charge fee
...
//
//        return
//            (result.traderList, result.paperChangeList, result.creditChangeList);
//    }
//}
//
...
//    }
//
//    function setMaxPositionAmount(uint256 newMaxPositionAmount)
//        external
//        onlyOwner
//    {
